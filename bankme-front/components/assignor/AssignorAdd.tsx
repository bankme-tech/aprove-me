import { addAssignor } from "@/services/assignor";
...
export default function AddAssignor() {
  ...
  const handleClick = async () => {
    ...
    const request = await addAssignor(JSON.parse(token), assignor);
  ...
  return (    
    <div>
      ...
      <form className="bg-gray-800 mt-16">
        ...
        <button onClick={handleClick} type="button" className="my-5 text-white bg-blue-600 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center">Adicionar Cedente</button>
      </form>
