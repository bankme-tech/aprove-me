...
export class UpdatePayableByIdInputDTO {
  ...
  constructor(input: unknown) {
    ...
    if (!("assignorId" in input)) {
      throw new ValidationError("assignorId is required");
    }

    if (typeof input.assignorId !== "string") {
      throw new ValidationError("assignorId must be a string");
    }

    if (!isUUID(input.assignorId)) {
      throw new ValidationError("assignorId is not a valid UUID");
    }
