...
export const PAYABLE_DEAD_LETTER_INJECT_TOKEN = "PAYABLE_DEAD_LEETER_INJECT_TOKEN ";
...
export class PayableDeadLetterQueueProvider {
  public constructor(
    @Inject(PAYABLE_DEAD_LETTER_INJECT_TOKEN) private readonly client: ClientProxy,
  ) { }

  /** Send payable to {@link PAYABLE_BATCH_QUEUE}. */
  async send(msg: PayableBatchMessage) {
    const sendDeadLetter$ = this.client.send(ROUTE_PAYABLE_BATCH_DEAD_LETTER, msg);
    return sendDeadLetter$.toPromise();
  }
}

/** Get consumer that process data asynchronously. */
export function getPayableBatchDLQConsumer(conn: RabbitmqConnection): ClientOptions {
...
export function getPayableBatchDLQProducer(conn: RabbitmqConnection): ClientProviderOptions {
